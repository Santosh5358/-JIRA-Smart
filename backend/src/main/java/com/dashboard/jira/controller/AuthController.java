...
public class AuthController {
     ...
     * Optional: { ..., "jiraBaseUrl": "https://jira.company.com/" }1
        */
    @PostMapping("/login")
    public ResponseEntity<Map<String, Object>> login(@RequestBody Map<String, String> credentials) {

        String username = credentials.get("username"); // should be email for Jira Cloud
        String token = credentials.get("token");       // Jira API token
        ...
        if (username == null || username.isBlank() || token == null || token.isBlank()) {
            return ResponseEntity.badRequest().body(Map.of(
                    "success", false,
                    "error", "Username (email) and API token are required."
            ));
        }
        ...
        try {
            ...
            String displayName = user.has("displayName")
            ...
            String emailAddress = user.has("emailAddress")
                    ? user.get("emailAddress").asText()
                    : username;

            // ✅ Generate JWT (store token safely if needed)
            ...
            return ResponseEntity.status(401).body(Map.of(
                    "success", false,
                    "error", "Invalid JIRA credentials. Make sure you're using email + API token."
            ));
